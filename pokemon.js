(function () {
  ...
  // Randomly select two pokemons indexs.
  var pokemonIndex1 = random(0, pokemonList.length - 1);
  var pokemonIndex2 = random(0, pokemonList.length - 1);

  // Get selected pokemons.
  var pokemon1 = pokemonList[pokemonIndex1];
  var pokemon2 = pokemonList[pokemonIndex2];
  // Print them.
  console.log(pokemon1.index + ': ' + pokemon1.name + ' (' + pokemon1.types.join(', ') + ')');
  console.log(pokemon2.index + ': ' + pokemon2.name + ' (' + pokemon2.types.join(', ') + ')');
  // join first half of first pokemon name and second half of second name.
  function joinNames(pok1,pok2) {
    var name1 = pok1.name.substring(0,Math.floor(pokemon1.name.length/2));
    var name2 = pok2.name.substring(Math.floor(pokemon1.name.length/2));
    return name1.concat(name2);
  }
  // get random type from two pokemons
  function getRandomType(pok1,pok2) {
    var types = pok1.types;
    types = types.concat(pok2.types);
    return types[random(0, types.length - 1)];
  }
  // print more random name
  console.log("Joined name: " + joinNames(pokemon1,pokemon2) + '-' + 'the' + '-' + getRandomType(pokemon1, pokemon2));
